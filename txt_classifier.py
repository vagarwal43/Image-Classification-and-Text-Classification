...
wandb.init(project="Gen-AI_HW0",name="text_classification",config={
    "epochs":5, "batch_size":8, "learning_rate":5, "MAX_LEN":1024,
    "Embed_dim":64
})
...
def collect_article_lengths(dataset):
  lengths = []
  for idx in range(len(dataset)):
      text = dataset.data_frame.loc[idx, "article"]
...
def get_data():    
    ...
    article_lengths = collect_article_lengths(train_data)
    wandb.log({"raw_article_lengths_histogram": wandb.Histogram(article_lengths)})
...
def train_one_epoch(dataloader, model, criterion, optimizer, epoch):
    ...
    for idx, (text, label) in enumerate(dataloader):
        ...
        if idx % log_interval == 0 and idx > 0:
            ...
            wandb.log({"train_accuracy": accuracy, "train_loss": loss.item(), "batch": idx, "epoch": epoch})
            print(
...
def evaluate(dataloader, model, criterion, epoch):
    ...
    with torch.no_grad():
        for idx, (text, label) in enumerate(dataloader):
            ...
            total_count += label.size(0)
        accuracy = total_acc / total_count
    wandb.log({"val_accuracy": accuracy, "val_loss": loss.item(), "epoch": epoch})
    return total_acc / total_count

def main():
    corpus_info, train_dataloader, val_dataloader, test_dataloader = get_data()

    model = TextClassificationModel(corpus_info.vocab_size, EMBED_DIM, corpus_info.num_labels, hidden_dim=128, num_layers=2).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        accu_val = evaluate(val_dataloader, model, criterion, epoch)
